...
function evaluateFormula() {
	...
	// Mostramos formulas
	d3.select('#subformula-container').style('display', 'inline');
	currentSubformula
		.classed('inactive', false);
  ...
  if(newMode === MODE.EDIT ) {
    ...
    evalOutput.classed('inactive', true);
		if (newMode === MODE.TREE){
			...
			currentFormula.classed('inactive', true);
			currentSubformula.classed('inactive', true);
			d3.select('#editing-info').style('display', 'none');
			d3.select('#info-box').style('display', 'none');
			d3.select('#subformula-container').style('display', 'none');
		} else {
			d3.select('#editing-info').style('display', 'block');
		}
	} else return;
...
function toggleFullScreen() {
    ...
    // const elem = document.getElementById("app-body")
